...
Double MyRand(unsigned int init= 99) {
	...
	return ((unsigned int)(next/65536) % 32768 ) / 32767.;
}
...
inline Double KMRlum::gluon(Double x, Double q2 ) const
{
  // Standard parton distributions.
    return beamPtr->xf(21, x, q2);
}

pair<Double,Double> KMRlum::gluonPairChic(Double x, Double q) const
{
  ...
  if(qsq  > q0) {
    Double fPlus  = gluon(x, qsq+eps1);
    Double fMinus = gluon(x, qsq-eps1);
    ...
    return make_pair(f, Df);
     ...
     Double glu1p=  gluon(x, qp); 
     Double glu1pp= gluon(x, qp+eps);
     Double glu1m=  gluon(x, qm);
     Double glu1mp= gluon(x, qm+eps);
...
Double KMRlum::LeifGluonChicUnintegrated(Double x, Double q, Double y, Double Int) const
{
  Double weight = 1;

  //Double y = pow(x/4,gRandom->Uniform(1) ); //importance sampling

  /*
  const Double a = 1.9;
  const Double b = 0.5;
  const Double Max = pow(a,a)*pow(b,b) / pow(a+b,a+b);
  const Double Int = TMath::Beta(a+1,b+1)*TMath::BetaIncomplete(1-x/4, b+1, a+1);

  Double y, wg, temp;
  do {
    y = Uniform(x/4,1);
    temp = Uniform(0,Max);
    wg = pow(y,a) * pow(1-y,b);

  } while(temp > wg); 


  Double weight = 16*y/M_PI *sqrt(y*(1-y));
  weight *= Int/wg;
  */
  ...
  //Double y = x/4. + (1-x/4.) *rnd;
  //weight *= 1 - x/4; //Size of interval

  weight *= 16*y/M_PI *sqrt(y*(1-y));

  //weight *= y* log(x/4);

  Double split = SplittingAlphaS( 2*log(q) );

  Double g,Dg;
  pair<Double,Double> res;
  res = gluonPairChic(x/4/y, q);
  g  = res.first;
  Dg = res.second;
...
void KMRlum::GetYint(Double x, Double &y, Double &Int)
{
...
vector< complex<Double> > KMRlum::LumSqrtRandom(Double M, Double y, Double Mju, Double MjuLoop, const Vec2 &p1, const Vec2 &p2) 
{
  ...
  for(int i=0; i < 4; ++i) {
    ...
    weightNow *=  max(LeifGluonChicUnintegrated(x1, q1n, y1, Int1), 0.0) *
                  max(LeifGluonChicUnintegrated(x2, q2n, y2, Int2), 0.0);
    ...
    weightNow *= exp(-0.5*( InsideSudakov(LnQ21, LnMju2)+InsideSudakov(LnQ22, LnMju2) ));
...
void KMRlum::LuminosityRand(Double M, Double y, Double Mju, Double MjuLoop, Vec2 &p1, Vec2 &p2, complex<Double> lum[], complex<Double> lumC[] ) 
{
  const int Niter = 20;
  ...
  for(int k=0; k < Niter; ++k) {
    ...
    lum1Temp = LumSqrtRandom(M, y, Mju, MjuLoop, p1, p2);
    lum2Temp = LumSqrtRandom(M, y, Mju, MjuLoop, p1, p2);
    for(int i=0; i< 3; ++i) {
...
Double KMRlum::LuminosityInc(Double M, Double y, Double Mju)
{
  Double sSqrt = sqrt(s);
  ...
  return gluon(x1,Mju*Mju) * gluon(x2,Mju*Mju); 

}

Double KMRlum::SplittingAlphaS(Double  LnScale2) const
{
  //cout <<"alphaS "<<  2*M_PI*alphaStrong(2*log(91) ) << endl;
  return alphaStrong(LnScale2) * Splitting( exp(0.5*LnScale2));

}

//Alpha Strong over 2pi
Double KMRlum::alphaStrong(Double LnScale2) const
{
...
Double KMRlum::Splitting(Double Kt) const
{
...
Double KMRlum::InsideSudakov(Double LnQ2, Double LnM2) const
{
  Double norm;
  if(LnQ2 < LnM2)
    norm = 1;
  ...
  if(start < end) {
    for(int i= start; i<nBP-1 && i<end; ++i)
      result += integral(&KMRlum::SplittingAlphaS, breakPoints[i], breakPoints[i+1], err );
  }

  if(start != -1) {
      result += integral(&KMRlum::SplittingAlphaS, LnQ2, breakPoints[start], err );
      result += integral(&KMRlum::SplittingAlphaS, breakPoints[end],   LnM2, err );
  }
  else {
      result += integral(&KMRlum::SplittingAlphaS, LnQ2, LnM2, err );
  }
...
Double KMRlum::integral( Double (KMRlum::*func)(Double x) const, Double xmin, Double xmax, Double &err ) const
{
  ...
  valuesG[0] = (this->*func)(xmean);
  for(int i=1; i < 4; ++i) {
    valuesG[i] = (this->*func)(xmean + nodsG7[i]*xdelta) +  (this->*func)(xmean - nodsG7[i]*xdelta);
  }
  for(int i=0; i < 4; ++i) {
    valuesK[i] = (this->*func)(xmean + nodsK15[i]*xdelta) + (this->*func)(xmean - nodsK15[i]*xdelta);
  }
...
void KMRlum::SaveIntegral(STATUS &st, Double xmin, Double xmax, Double Int) const
{
  Double Mred = x1*x2;
...
Double KMRlum::NoEmProbability(Double _x1, Double _x2, Double LnQ2, Double LnM2, bool backward) 
{
  x1 = _x1;
  x2 = _x2;

  Double res = exp( - 2.*InsideSudakov(LnQ2, LnM2) );

  if(backward) {
		res *=
		gluon(x1, exp(LnQ2) ) / gluon(x1, exp(LnM2) ) *
		gluon(x2, exp(LnQ2) ) / gluon(x2, exp(LnM2) );
	}
